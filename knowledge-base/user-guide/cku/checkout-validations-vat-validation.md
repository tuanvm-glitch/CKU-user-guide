...
1. Go to **Shopify Admin → Settings → Checkout → Customize** Or: **Online Store → Themes → Customize → select "Checkout and customer accounts" page**
...
{% hint style="info" %}
The description reads: _"Orders will be rejected if customer information is missing or invalid."_ This is exactly what enables the VAT validation to block checkout when the buyer leaves the field empty or enters an invalid format.
{% endhint %}
